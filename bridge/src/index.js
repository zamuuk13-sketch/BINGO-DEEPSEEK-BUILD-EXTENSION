...
function fail(id, code, message) {
  send({ protocol: PROTOCOL_VERSION, id, ok: false, error: { code, message } });
}
...
async function execute(command) {
  ...
  try {
    if (op === 'project.create') {
      const name = safeProject(command.name);
      const projectDir = ensureProject(name);
      return send({ protocol: PROTOCOL_VERSION, id, ok: true, op, project: name, path: projectDir });
    }

    const projectName = safeProject(project);
    ...
    if (op === 'fs.mkdir') {
      const { target } = safePath(projectName, relative);
      fs.mkdirSync(target, { recursive: true });
      return send({ protocol: PROTOCOL_VERSION, id, ok: true, op, path: relative });
    }
    ...
    if (op === 'fs.write') {
      ...
      return send({ protocol: PROTOCOL_VERSION, id, ok: true, op, path: relative, bytes: Buffer.byteLength(String(content ?? ''), 'utf8') });
    }

    if (op === 'fs.read') {
      const { target } = safePath(projectName, relative);
      const value = fs.readFileSync(target, 'utf8');
      return send({ protocol: PROTOCOL_VERSION, id, ok: true, op, path: relative, content: value });
    }

    if (op === 'fs.list') {
      const { target } = safePath(projectName, relative || '');
      const entries = fs.existsSync(target) ? fs.readdirSync(target, { withFileTypes: true }).map(e => ({ name: e.name, type: e.isDirectory() ? 'directory' : 'file' })) : [];
    ...
    if (op === 'fs.rename') {
      const a = safePath(projectName, from).target;
      const b = safePath(projectName, to).target;
      fs.mkdirSync(path.dirname(b), { recursive: true });
      fs.renameSync(a, b);
      return send({ protocol: PROTOCOL_VERSION, id, ok: true, op, from, to });
    }
    ...
    if (op === 'process.run') {
      ...
      if (!executable || executable.includes('..')) throw new Error('Comando inválido');
      ...
      child.on('close', code => {
        clearTimeout(timer);
        send({ protocol: PROTOCOL_VERSION, id, ok: code === 0, op, exitCode: code, stdout, stderr });
      });
      return;
    }
  } catch (error) {
    fail(id, 'EXECUTION_ERROR', error.message);
  }
}
...
process.stdin.on('data', chunk => {
  ...
  while (input.length >= 4) {
    ...
    try {
      ...
      fail(null, 'INVALID_JSON', error.message);
    }
  }
});
