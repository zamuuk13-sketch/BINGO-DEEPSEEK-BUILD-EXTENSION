...
def cpu_info(project=None):
    result={"logicalCpus":os.cpu_count() or 1,"processor":platform.processor()}
    if os.name=="nt":
        try: result["wmic"]=run_readonly(["wmic","cpu","get","Name,NumberOfCores,NumberOfLogicalProcessors","/value"],5)
        except Exception as exc: result["wmicError"]=str(exc)
    return result

def memory_info(project=None):
    if os.name=="nt":
        try: return {"source":"wmic","data":run_readonly(["wmic","OS","get","TotalVisibleMemorySize,FreePhysicalMemory","/value"],5)}
...
def network_info(project=None):
    ...
    try: dns=True; socket.gethostbyname("localhost")
    except Exception: dns=False
    return {"hostname":host,"addresses":addresses,"localhostResolved":dns}

def processes_info(project=None):
    command=["tasklist","/FO","CSV","/NH"] if os.name=="nt" else ["ps","-eo","pid,comm"]
    try: return {"source":command[0],"data":run_readonly(command,10)}
    except Exception as exc: return {"source":command[0],"error":str(exc)}
...
def project_info(project):
    project=project_name(project); path=project_dir(project); files=dirs=0; ignored={".git",".godot","node_modules",".venv","venv","build","dist",".bingo"}
    for item in path.rglob("*"):
        if any(part in ignored for part in item.parts): continue
