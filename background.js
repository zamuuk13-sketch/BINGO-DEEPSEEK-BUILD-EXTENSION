...
function classifyDiagnostic(result) {
  ...
  return {
    status:result?.exitCode===0?'warning':'failed',
    category:matches[0] || (result?.exitCode===0?'warning':'unknown_failure'),
    summary:result?.exitCode===0?'O processo terminou, mas ha sinais que merecem verificacao.':'O processo falhou; use o diagnostico para orientar a correcao.',
