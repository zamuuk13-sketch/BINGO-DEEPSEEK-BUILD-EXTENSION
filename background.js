...
async function pythonCall(tool, args) {
  ...
  const response = await fetchWithTimeout(`${PYTHON_BRIDGE}/tool`, {
    ...
    body: JSON.stringify({
      id: `bingo-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      op: tool,
      ...args
    })
  });
  ...
  try {
    ...
    throw new Error(`Bridge retornou HTTP ${response.status} sem JSON valido.`);
  }

  if (!response.ok || !data.ok) {
    throw new Error(data.error || `Python Bridge HTTP ${response.status}`);
  }
  return data;
}
...
async function bridgeHealth() {
  ...
  if (!response.ok || !data.ok) throw new Error(data.error || `Bridge HTTP ${response.status}`);
  return data;
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'BINGO_TOOL_CALL') {
    if (typeof tabId !== 'number') {
      sendResponse({ok:false, error:{message:'Aba invalida.'}});
      return true;
    }
    if (!TOOLS.has(msg.tool)) {
      sendResponse({ok:false, error:{message:`Ferramenta nao permitida: ${msg.tool}`}});
      return true;
    }
    ...
    pythonCall(msg.tool, msg.args || {}).then(result => {
      ...
      const message = error?.name === 'AbortError'
        ? 'Tempo limite da bridge excedido.'
  ...
  if (msg.type === 'COMMAND_AUDIT') {
    session.history.push({
      at:Date.now(), id:msg.id || null, op:msg.op || 'unknown',
      ok:!!msg.ok, detail:msg.detail || ''
    });
