...
(() => {
  ...
  const TOOLS = [
    ...
    ['terminal.run_script','terminal','Executa um script do projeto com controle de diretório.',['project','path']],
    ['terminal.capture','terminal','Executa um comando capturando saída estruturada.',['project','command']],
    ['terminal.pipe','terminal','Executa uma cadeia controlada de comandos do projeto.',['project','commands']]
  ];

  function registerAll() {
    for (const [name,category,description,args] of TOOLS) {
      registry.register({
        name, category, description, args,
        permission: category === 'terminal' ? 'terminal' : null,
        tags:['stage13','terminal','shell','process'],
        source:'stage13', version:VERSION
      }, {replace:true});
    }
  }

  function capabilities() {
    return TOOLS.map(([name,category,description,args]) => ({name,category,description,args}));
  }

  registerAll();
  globalThis.BingoTerminalToolsV13 = Object.freeze({
    version:VERSION,
