...
class BingoClient(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.config_store = ConfigStore(); self.project_path: Path | None = None; self.workspace: Workspace | None = None
        self.title(f"{APP_NAME} • {APP_VERSION}"); self.geometry("1360x800"); self.minsize(1050, 650); self.configure(bg=self.BG)
        self.protocol("WM_DELETE_WINDOW", self.on_close); self._configure_ttk(); self._build_ui(); self._restore_project()
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Abrir pasta", style="Bingo.TButton", command=self.choose_project).pack(side="right", padx=7, pady=9)
        ...
        for label, cmd in (("+ Arquivo", self.new_file), ("+ Pasta", self.new_folder), ("Renomear", self.rename_selected), ("Excluir", self.delete_selected), ("↻", self._refresh_tree)):
            ttk.Button(bar, text=label, style="Bingo.TButton", command=cmd).pack(side="left", padx=1)
        self.tree = ttk.Treeview(explorer, style="Bingo.Treeview", show="tree"); self.tree.pack(fill="both", expand=True, padx=7, pady=(0, 7))
        self.tree.bind("<Double-1>", self.on_tree_open); self.tree.bind("<Button-3>", self.on_tree_context); self.tree.bind("<<TreeviewOpen>>", self.on_tree_expand)
        ...
        tk.Label(right, text="EDITOR", bg=self.PANEL, fg=self.MUTED, font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=14, pady=(20, 8))
        tk.Label(right, text="Ctrl+S  •  Salvar\nCtrl+F  •  Pesquisar\nDuplo clique  •  Abrir arquivo", bg=self.PANEL_2, fg=self.MUTED, justify="left", anchor="w").pack(fill="x", padx=12, ipady=12, ipadx=10)
        ...
        tk.Label(bottom, text=f"Workspace • Editor • {APP_VERSION}", bg=self.PANEL, fg=self.MUTED, font=("Segoe UI", 8)).pack(side="right", padx=12)
    ...
    def open_project(self, path: Path):
        self.project_path = path.resolve(); self.workspace = Workspace(self.project_path)
        self.config_store.data["last_project"] = str(self.project_path); self.config_store.save()
        self.project_label.config(text=self.project_path.name or str(self.project_path)); self.workspace_value.config(text=str(self.project_path), fg=self.TEXT)
        self.set_status(f"Workspace aberto: {self.project_path}"); self._refresh_tree()
    ...
    def _refresh_tree(self):
        ...
        try:
            entries = self.workspace.list_children(); [self._insert_entry(root, p) for p in entries]; self.item_count.config(text=f"{len(entries)} itens na raiz")
        except OSError as exc: self.set_status(f"Erro ao listar: {exc}")

    def _insert_entry(self, parent, path: Path):
        iid = f"path:{path}"
        if path.is_dir():
            self.tree.insert(parent, "end", iid=iid, text=f"📁  {path.name}", values=(str(path),))
            self.tree.insert(iid, "end", iid=f"dummy:{path}", text="carregando…")
    ...
    def _ask_name(self, title, prompt, initial=""):
        ...
        tk.Label(dialog, text=prompt, bg=self.BG, fg=self.TEXT).pack(anchor="w", padx=20, pady=(18, 6))
        value = tk.StringVar(value=initial); entry = ttk.Entry(dialog, textvariable=value); entry.pack(fill="x", padx=20); entry.focus_set(); result = []
        def accept():
    ...
    def show_settings(self):
        ...
        tk.Label(dialog, text="Permissões serão usadas pelo Agent Runtime.", bg=self.BG, fg=self.MUTED).pack(anchor="w", padx=24)
        ...
        scope = tk.StringVar(value=self.config_store.data["permissions"]["scope"]); ttk.Combobox(box, textvariable=scope, values=("project", "computer"), state="readonly").pack(anchor="w", padx=14, pady=(0, 14))
        def save(): self.config_store.data["permissions"]["scope"] = scope.get(); self.config_store.save(); dialog.destroy(); self.set_status("Configurações salvas")
